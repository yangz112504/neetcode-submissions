class Solution:
    def maxSubarraySumCircular(self, nums: List[int]) -> int:
        ...
        if totalArraySum == maxMinSum: #edge case where the whole array is negative, then return maxSum
            return maxSum

        circularSum = totalArraySum - maxMinSum # wrap around sum

        return max(circularSum, maxSum) #return max of wraparound sum vs nonwraparoundsum