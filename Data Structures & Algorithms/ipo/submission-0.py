class Solution:
    def findMaximizedCapital(self, k: int, w: int, profits: List[int], capital: List[int]) -> int:
        ...
        maxProfitHeap = []
        for i in range(k):
            while minCapitalHeap:
                requiredCapital = minCapitalHeap[0][0]
                if w < requiredCapital:
                    break # can't afford anymore projects
                requiredCapital, profit = heapq.heappop(minCapitalHeap)
                heapq.heappush(maxProfitHeap, -1 * profit)
            if not maxProfitHeap:
                break # if nothing in maxProfit heap that means no more profit can be earned so return current capital
